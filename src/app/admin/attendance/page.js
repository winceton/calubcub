...
import SF4Modal from '../../../app/forms/SF4Modal'
...
const Page = () => {
  ...
  return (
    <div className='ml-[6rem] mt-[5rem] bg-gray-50 min-h-auto p-6 rounded-lg shadow-lg'>
      <NavbarAdmin route={'admin'} routeName1={'attendance'} />
      ...
      <div className='flex justify-between items-center'>
        ...
        <div className='text-white cursor-pointer py-2 px-4 rounded-lg bg-[#820000] hover:bg-[#6b1919] transition-all text-lg mr-4' onClick={() => setOpenModal(true)}>View SF4</div>
      </div>
                  ...
                  <tr key={section.id} className={`${index % 2 === 0 ? 'bg-gray-100' : 'bg-gray-200'} border-t border-gray-300 hover:bg-gray-300 transition-all duration-300`}>
                    ...
                    <td className='px-6 py-3'>
                      <button
                        ...
                        disabled={!section.adviser}
                        className={`px-4 py-2 rounded-lg shadow-md transition-all duration-300 ${section.adviser?.first_name
      ...
      {openModal && <SF4Modal onClose={() => setOpenModal(false)} />}
    </div>
  )
}
