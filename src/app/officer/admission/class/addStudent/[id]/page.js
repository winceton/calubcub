...
const addStudent = () => {
  ...
  const handleChange = (e, field, parent, subParent) => {
    ...
    if (field === 'birthdate') {
      ...
      // Adjust age if birthdate is after June 1
  ...
  const handleInsert = async (e) => {
    ...
    try {
      // Loop through each student and insert their 'lrn' as a new row
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          parents: {
            father: {
              firstname: student.parents.father.firstname,
              middlename: student.parents.father.middlename,
              lastname: student.parents.father.lastname,
            },
            mother: {
              firstname: student.parents.mother.firstname,
              middlename: student.parents.mother.middlename,
              lastname: student.parents.mother.lastname,
            },
          },
          guardian: {
          ...
          remarks: {
            sf1: student.remarks.sf1,
            sf2: student.remarks.sf2
          },
          section_id: id,
        }])
      ...
      setMunicipalities([])
      setBarangays([])
      router.push(`/admin/classes/class/${id}`)
    } catch (err) {
      console.error('Error inserting LRNs:', err);
    }
  };
  ...
  return (
    <div className="ml-[5rem] mr-[1rem] md:ml-[10rem] md:mr-[5rem] lg:ml-[15rem] xl:max-w-4xl xl:mx-auto lg:mr-[10rem] mt-[5rem] p-6 bg-white shadow-md rounded-lg">
      <NavbarAdmin route={'officer'} routeName1={'admission'} routeName2={`Grade ${section?.class_id} ${section?.section_name}`} routeName3={'Add'} />
      <h2 className="text-3xl font-bold mb-8 text-center text-[#820000]">Student Information</h2>
      ...
      <div className="flex justify-end gap-4 mt-8">
        <button className="bg-[#820000] text-white px-6 py-3 rounded-md hover:bg-[#a72b2b] transition-colors" onClick={handleInsert}>Add Student</button>
      </div>
    </div>
  );
}
